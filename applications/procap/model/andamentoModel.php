	...
	function formValidate($fields){
		...
		foreach ( $arquivos as $arquivo ) {
			if ($arquivo->_value ['size'] > 0) {
				...
				if (! in_array( $_FILES [$arquivo->getName()] ['type'], $types )) {
					$this->controller->getEnv()->formErrorMessages [$arquivo->getName()] = 'somente PDF e DOC';
					$this->form->getElement( $arquivo->getName() )->setAttribute( 'class', 'error' );
					$ret = false;
				}
				if ($_FILES [$arquivo->getName()] ['size'] > 5 * 1024 * 1024) {
					$this->controller->getEnv()->formErrorMessages [$arquivo->getName()] = 'm&aacute;ximo 5MB';
					$this->form->getElement( $arquivo->getName() )->setAttribute( 'class', 'error' );
					$ret = false;
				}
			}
