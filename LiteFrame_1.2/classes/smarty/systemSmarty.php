...
class classes_smarty_systemSmarty extends Smarty
{
   ...
   function smartyExecute(){
      if( defined(STATIC_URL)) $this->assign('static_url',STATIC_URL) ;
      if( defined(SITE_ROOT)) $this->assign('site_url',SITE_ROOT) ;

      $this->assign('messages',$this->controller->messages->getMessages());
      $this->assign('crudAction',$this->controller->getAction()->getCrudAction()  );
            ...
            if($form->formValidate) {
               ...
               foreach ($frm->_elements  as $elem ){
                  if($elem->getType()=='select'){
                     $elem->_options = array_merge(array(''=>array('text' => '','attr' => array('value' => ''))),$elem->_options);
                  }
               }

               $frm = $form->toArray();
               //pr($form);
               $this->assign('jqueryInputMaskScript', "<script type='text/javascript'> $(document).ready(function() { jQuery(function($) {".$form->jqueryInputMaskScript ." });  }); </script>" );
   ...
   function display($OriginalTemplate){
      //pr($this->lists);
      if(!empty($this->lists)){
         //pr($OriginalTemplate);
