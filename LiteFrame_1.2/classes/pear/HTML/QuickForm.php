...
class HTML_QuickForm extends HTML_Common
{
    ...
    * @throws   HTML_QuickForm_Error
    */
   function addFormRule($obj,$rule)
   {
      $this->formRules[] = array('method' => $rule,'obj' => $obj);
   }
      ...
      foreach ($this->_rules as $elementName => $rules) {
         foreach ($rules as $rule) {
            if ('client' == $rule['validation']) {
               ...
               $rule['message'] = $this->getElement($elementName)->_label .' '. strtr($rule['message'], $js_escape);
   ...
   function toArray($collectHidden = false)
   {
      ...
      foreach ($array['elements'] as $key=>$element ){
         $array['elements'][$element['name']] = $element;
      }
   ...
   function getHtml($template){
      ...
      $view = new classes_smarty_systemSmarty(null);
      $view->assign('jsValidationScript',$fa['javascript']);
      $view->assign('formOptions',$fa['attributes']);
      $view->assign('message','em teste');

      foreach ($fa['elements'] as $element ){
         $view->assign($element['name'].'_label',$element['label']);
         $view->assign($element['name'].'_html',$element['html']);
      }

      echo  $view->toHtml($template);
   }


   function addButton($type, array $options = null , $name=null, $label=null){
      switch($type){
         case 'button':
            $this->addElement($type, $name , $label, $options );
            break;
         case 'insert':
            $this->addElement('submit', INSERT_BUTTON_NAME , INSERT_BUTTON_LABEL , $options );
            break;
         case 'update':
            $this->addElement('submit',UPDATE_BUTTON_NAME, UPDATE_BUTTON_LABEL , $options );
            break;
         case 'delete':
            $this->addElement('button', DELETE_BUTTON_NAME,  DELETE_BUTTON_LABEL , $options );
            break;
         case 'new':
            $this->addElement('button', NEW_BUTTON_NAME ,  NEW_BUTTON_LABEL , $options );
            break;
         case 'list':
            $this->addElement('button', LIST_BUTTON_NAME ,  LIST_BUTTON_LABEL , array('onclick' => "window.location='".$_SERVER["HTTP_REFERER"]."'"  ) );
            break;
         default :
            throw new Exception('Undefined button type: '.$type);
      }
   }
